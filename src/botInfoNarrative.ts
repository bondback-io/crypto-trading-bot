...
export function renderBotInfoSectionArticles(slots: BotInfoSlots): string {
  ...
  return `
      ...
      <article class="botinfo-card" id="botinfo-sec-risk" data-botinfo-section="risk">
        ...
        <div class="botinfo-callout"><strong>Multi-TF S/R + Scalper watch (Mode B):</strong> Scalper / Momentum Burst / Reversal only immediate-buy when price is already at multi-TF support confluence (≥2 TFs including 15m/30m/1h/4h) in the <strong>mid-MC band ($150k–$1M)</strong>. Otherwise they park on the Scalper-family watchlist until touch + reclaim (~1.2%). <strong>Scalper owns mid-band support reclaim</strong> with quick TP harvest; microcaps &lt;$150k stamp Migration / Reversal. Dip ownership starts at catalog min <strong>$1M–$500M</strong> (eased floors are not ownership). Mutual exclusion with Dip is by <em>active dip watch</em> (Fib dips), not a hard $500k cut. Soft-prefers Scalper when armed or reclaiming at S unless reversal wick or MB volume-expansion dominate. While Scalper Fast Recovery is Stage 0–1, Mode B stamps Scalper only on true support reclaim. <strong>Armed watches</strong> soft-pass Gatekeeper advisories and <strong>bypass scanner mint cooldown</strong> on trigger handoff (hard safety still final). Profile TA Playbooks retuned per lane; Soft/Hard score the buy.</div>
