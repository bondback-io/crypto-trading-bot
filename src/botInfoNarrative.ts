...
export function renderBotInfoSectionArticles(slots: BotInfoSlots): string {
  ...
  return `
      ...
      <article class="botinfo-card" id="botinfo-sec-risk" data-botinfo-section="risk">
        ...
        <div class="botinfo-callout"><strong>Multi-TF S/R + Scalper watch (Mode B):</strong> Scalper / Momentum Burst / Reversal only immediate-buy when price is already at multi-TF support confluence (≥2 TFs including 15m/30m/1h/4h). Otherwise they park on the Scalper-family watchlist until touch + reclaim (~1.2%). <strong>Scalper prioritizes support reclaim</strong> / near multi-TF support for small-MC (≤$180k) — soft lane boost + watch prefer unless reversal wick or MB volume-expansion dominate; late chase away from support is tightened. While Scalper Fast Recovery is Stage 0–1, Mode B stamps Scalper only on true support reclaim (not MC-band alone) so MB/Reversal share the strip — recovery still throttles admits (concurrency / size / vol), not watchlist admission. <strong>Armed watches soft-pass</strong> Gatekeeper advisories (hard safety still final) and stamp <code>armedWatch</code> / <code>entryPath: armed_trigger</code> through buy → PCL. Scalper discretionary share caps ~35% when WR &lt;45% or recovery stage ≤1; armed reclaim bypasses. Profile TA Playbooks do not gate Mode B pickup; Soft/Hard score the buy. Not Dip Buyer: Fib dips stay on Dip (~1.5% reclaim). Shared support-reclaim detector fails soft if S/R is missing.</div>
