  ...
  return `
      <article class="botinfo-card" id="botinfo-sec-lifecycle" data-botinfo-section="lifecycle">
        ...
        <div class="botinfo-lifecycle-stages">
          ...
          ${lifecycleStage(
            ...
            'A candidate appears from the Market Scanner (Watchlist → Scanner) or a tracked smart-wallet buy. Early kills can stop here (scanner/copy OFF, risk halt, denied mint, pump.fun-only gate).',
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Watchlist → Activity (signals) · Micro Bots → HMC Gatekeeper / Classifier, MARL, Profile RL, Peak Protect, Profit Capture Layer · Overview → lane fight log · Stats → Agent Decision Log · Back Up → learning journal.</p>
        <div class="botinfo-actions">
          ${btn('scanner', 'Open Watchlist')}
          ${btn('microbots', 'Open Micro Bots')}
          ${btn('overview', 'Open Overview')}
          ${btn('botperf', 'Open Stats')}
        </div>
      </article>
      ...
      <article class="botinfo-card" id="botinfo-sec-overview" data-botinfo-section="overview">
        ...
        <ul>
          ...
          <li><strong>Stats</strong> — rankings, Expectancy Lift, Trade Craft, Learning, <em>Learning Metrics</em> (per-profile readiness / EMA / capture / blockers), Agent Decision Log, Export Data (system dump + <em>Learning Report</em>), and <em>RPC</em> (pool health + endpoint table) (main nav, right of Micro Bots).</li>
      ...
      <article class="botinfo-card" id="botinfo-sec-risk" data-botinfo-section="risk">
        ...
        <div class="botinfo-callout"><strong>Multi-TF S/R + Scalper watch (Mode B):</strong> Scalper / Momentum Burst / Reversal only immediate-buy when price is already at multi-TF support confluence (≥2 TFs including 15m/30m/1h/4h) in the <strong>mid-MC band ($150k–$800k)</strong>. Otherwise they park on the Scalper-family watchlist until touch + reclaim (~1.2%). <strong>Scalper owns mid-band support reclaim</strong> with quick TP harvest; microcaps &lt;$150k stamp Migration / Reversal. Mutual exclusion with Dip is by <em>active dip watch</em> (Fib dips), not a hard $500k cut. Soft-prefers Scalper when armed or reclaiming at S unless reversal wick or MB volume-expansion dominate. While Scalper Fast Recovery is Stage 0–1, Mode B stamps Scalper only on true support reclaim. <strong>Armed watches</strong> soft-pass Gatekeeper advisories and <strong>bypass scanner mint cooldown</strong> on trigger handoff (hard safety still final). Profile TA Playbooks retuned per lane; Soft/Hard score the buy.</div>
        <div class="botinfo-callout"><strong>Entry-style DNA + late-chase:</strong> Each micro-bot has primary / allowed / forbidden entry styles (reclaim, dip, trend pullback, SM confirm, etc.). Late chase (extended ~8–12% above support/Fib, or momentum with no nearby level) hard-zeros quality / Mirror / Trend / Dip and strongly penalizes Scalper / MB. Opens stamp <code>entryStyle</code> + <code>lateChaseAtEntry</code>; high-quality valid styles stretch PCL permission and delay PPP arm; marginal / late-chase tighten. Armed / medium-high reclaim harvest: early partial ~8–10% fraction 0.40–0.50, PPP arm ~75% of TP, permission ×1.5, block tiny green scratch if MFE ≥10%. Badges + Trade Craft Decisions KPIs + Zion call out elevated late-chase rates and armed-watch open rates.</div>
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Dashboard → Micro Bots → scroll to <em>Peak Profit Protection</em> card (near Profit Capture Layer). Open positions show Armed / Waiting. Stats → Agent Decision Log → filter Peak Protect. Per-bot arm/giveback: Micro Bots → profile Exit &amp; sizing.</p>
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Dashboard → Micro Bots → <em>Profit Capture Layer</em> card (directly under Peak Protect). Open-trade rows show permission / partial / PPP lines. Zion comments when PCL is active. Learning strength + family permission seconds are on that same card. <strong>Progress:</strong> Stats → <em>Trade Craft Progress</em> (Harvest tab + Combined) · full guide in <a href="#botinfo-sec-tradecraft" onclick="showBotInfoSection('tradecraft'); return false;">Trade Craft</a>.</p>
      ...
      <article class="botinfo-card" id="botinfo-sec-microbots" data-botinfo-section="microbots">
        ...
        ${slots.profilesGrid}
        <ul>
          <li><strong>Watchlists &amp; inventory</strong> — Dip (minors) · Steady Medium ($20–200M) · Steady Majors (≥$200M) · Trend Rider setup watch · Mode B (Scalper-family) · Graduation. Caps and mutual-exclude differ by band; medium/majors do not Trend-exclude Dip/Steady the way minors can.</li>
      ...
      <article class="botinfo-card" id="botinfo-sec-learning" data-botinfo-section="learning">
        ...
        <ul>
          ...
          <li><strong>Profile TA Playbooks</strong> — per-lane Off/Soft/Hard identity (HA, Fib/S-R, RSI/EMA/VWAP, patterns, optional whale). Soft = confirmation/conviction only; Hard = confluence gate. Does <em>not</em> gate Mode B / grad watchlist pickup — scores on the buy path after lane fight. Fast lanes (Scalper Soft+learning, MB Soft, Reversal Hard, Migration Soft) ship with catalog tools; Learning nudges tool weights / minConf only — never TP/SL or Peak Protect cores. Global Require TA remains scanner master.</li>
      ...
      <article class="botinfo-card" id="botinfo-sec-tradecraft" data-botinfo-section="tradecraft">
        ...
        <ul>
          ...
          <li>Craft never invents knobs. Tip: open Stats → <em>Trade Craft Progress</em> for charts + film table. Micro Bots → <em>Performance Power Cell</em> is a visual charge twin (same window) — not a second craft engine.</li>
        </ul>
        ...
        <ul>
          ...
          <li>Verify Harvest / Exits early→late on Stats → Trade Craft Progress (Combined or per bot).</li>
        </ul>
        <p class="botinfo-where"><strong>Where to find:</strong> Stats → <em>Trade Craft Progress</em> · Micro Bots → Peak Protect + Profit Capture Layer · Learning journal on Back Up · related chapters <em>Learning &amp; ML</em>, <em>Coaches &amp; Stack</em>, <em>Risk</em>.</p>
        <div class="botinfo-actions">
      ...
      <article class="botinfo-card" id="botinfo-sec-coaches" data-botinfo-section="coaches">
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Dashboard → Micro Bots → Profile routing <em>Learning</em> → <em>HMC Gatekeeper</em> card. Overview / Micro Bots lane fight log (HMC gate line). Stats → Agent Decision Log → source HMC Gatekeeper.</p>
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Micro Bots → <em>Learning</em> → Profile RL Agents card. Stats → <em>Learning Metrics</em> (readiness / EMA / blockers) · Agent Decision Log (Profile RL) · Learning Progress.</p>
        ...
        <p class="botinfo-where"><strong>Where to find:</strong> Micro Bots → Peak Protect + Profit Capture Layer cards · Risk chapter in this manual · open-position PCL/PPP lines · Stats → <em>Trade Craft Progress</em>.</p>
      ...
      <article class="botinfo-card" id="botinfo-sec-scanners" data-botinfo-section="scanners">
        ...
        <p>The <strong>Watchlist</strong> tab is the market universe, split into <em>Setups</em> / <em>Scanner</em> / <em>Activity</em>: autonomous scanner (Dex / GMGN / Birdeye + optional Jupiter trending), optional <strong>AlphaScan</strong> New/Soon/Bonded (default off), Pump.fun smart activity (early curve, near migration, migrations), playbooks, and re-entry watches.</p>
        <ul>
        ...
        <div class="botinfo-actions">${btn('scanner', 'Open Watchlist')}</div>
      </article>
      ...
      <article class="botinfo-card" id="botinfo-sec-execution" data-botinfo-section="execution">
        ...
        <p>Live buys/sells go through <strong>Jupiter</strong> swaps. Jupiter Tokens API also feeds organic score / trending for the scanner. Dual-lane RPC prefers free <strong>Helius</strong> (primary) + <strong>Alchemy</strong> (secondary / Zion) via <code>HELIUS_API_KEY</code> / <code>ALCHEMY_API_KEY</code> (or single <code>HELIUS_RPC_URL</code> / <code>ALCHEMY_RPC_URL</code>) with automatic failover to <code>RPC_URL</code>, public Solana, then <code>RPC_SECONDARY</code>.</p>
        <ul>
      ...
      <article class="botinfo-card" id="botinfo-sec-copy" data-botinfo-section="copy">
        ...
        <ul>
          ...
          <li><strong>Influencer Mirror</strong> (default OFF) — Add Wallet (GMGN enrich) + Import top 15 (30d PnL, GMGN primary) + Jupiter influencers (fail-soft). Soft Gatekeeper on copy buys; mirror-sell preferred vs PPP; Watch-tab Smart Mirror Watchlist (top 10 × 3 tokens with Copy CA / Jupiter, peach Copied badge, cross-hold +N, Add token). Copy / skip appears on Overview lane fight log under Smart Money Mirror. Tagged wallets stay pinned in soft-watch when master is ON. Copy size = profile risk sizing × wallet sizeMult — never influencer notional. Requires master ON + <code>smart_money_copy</code> + <code>smart_money_mirror</code>. Anti-rug / hard honeypot absolute.</li>
